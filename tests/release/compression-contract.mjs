...
export const COMPRESSION_SCOPE = 'foreground-compression-download/v3';
export const compressionCases = () => [
    { id: 'small-avif-standard', fixture: 'small', format: 'avif', compression: 'standard', ratio: 1 },
...
export function validateCompressionResult(result, specification) {
    ...
    if (specification.compression === 'standard') {
        assert.ok(result.quality == null && result.paletteColors == null, 'standard export must not expose compression controls');
    }
...
export function validateCompressionEvidence(evidence) {
    ...
    assert.deepEqual(evidence.avifPreviewCapability, { state: 'standard-only',
        noticeVisible: true, previewEnabled: false, directDownloadEnabled: true }, 'AVIF standard-only product policy');
    ...
    assert.deepEqual(evidence.largeAvifStandardOnly, { lossyDisabled: true, standardEnabled: true, previewDisabled: true, noDownload: true });
}
