...
export async function checkCompressionDownloads({ driver, editorWindow, selectFormat, waitForEnabled,
    ...
    for (const specification of evidence.registration.cases) {
        ...
        if (specification.id === 'small-avif-standard') {
            evidence.avifPreviewCapability = await driver.executeScript(() => {
                const notice = document.querySelector('[data-testid="avif-standard-only"]');
                return { state: 'standard-only', noticeVisible: !!notice && notice.getBoundingClientRect().height > 0,
                    previewEnabled: !!document.querySelector('[data-testid="export-preview"]'),
    ...
    const lossyDisabled = (await driver.findElements(By.css('[data-testid="compression-lossy"]'))).length === 0;
    const previewDisabled = (await driver.findElements(By.css('[data-testid="export-preview"]'))).length === 0;
    ...
    evidence.largeAvifStandardOnly = { lossyDisabled, standardEnabled, previewDisabled, noDownload };
