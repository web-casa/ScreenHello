...
test('packaged stylesheet ships the Radix theme the library renders', async ({ page }) => {
    // 宿主只引 rico-screenshot/style.css、不安装 @radix-ui/themes：
    // 包内的 Radix 控件必须仍然拿到 token 与组件样式。
    await page.goto('/?workspace=true');
    const first = page.locator('#consumer-a');
    await expect(first.locator('.shoteasy-app')).toBeVisible();
    ...
    const report = await page.evaluate(() => {
        ...
        for (const sheet of Array.from(document.styleSheets)) {
            try {
                ...
                // 跨源样式表读不到 cssRules；包内样式与页面同源，这里不会命中。
            }
        }
        const themed = document.querySelector('#consumer-a .radix-themes');
        const button = document.querySelector('#consumer-a .rt-Button');
        const wrapper = document.querySelector('#consumer-a .shoteasy-top-actions-radix');
        const topbar = document.querySelector('#consumer-a .shoteasy-topbar');
        return {
            hasTheme: Boolean(themed),
            gray12: themed ? getComputedStyle(themed).getPropertyValue('--gray-12').trim() : '',
            buttonRules: selectors.filter((selector) => selector.includes('.rt-Button')).length,
            // 渲染出来的控件必须真的有 Radix 外观（圆角来自 components.css）
            buttonRadius: button ? getComputedStyle(button).borderTopLeftRadius : '',
            // 桥接规则必须随包：Radix 的根主题带 min-height:100dvh，包裹层一旦参与布局就会撑高顶栏
    ...
    expect(report.gray12).not.toBe('');
    expect(report.buttonRules).toBeGreaterThan(0);
    if (report.buttonRadius) expect(report.buttonRadius).not.toBe('0px');
    expect(report.wrapperDisplay).toBe('contents');
    expect(report.topbarHeight).toBeLessThan(100);
});
...
test('packaged library ships no device artwork and degrades to no device model cards', async ({ page }) => {
    const requests = [];
    page.on('request', (request) => requests.push(request.url()));
    await page.goto('/?workspace=true');
    const first = page.locator('#consumer-a');
    await first.locator('.shoteasy-upload-card input[type=file]').setInputFiles({
        name: 'devices.png', mimeType: 'image/png', buffer: createPngFixture(64, 48),
    });
    await expect(first.locator('.shoteasy-editor-canvas')).toBeVisible();
    await first.getByRole('button', { name: '更多外框', exact: true }).click();
    const drawer = page.locator('.shoteasy-frame-drawer');
    await expect(drawer).toBeVisible();
    // Devices.css 机型素材不随 library 产物发布：抽屉里没有这些型号，也没有 devicescss 请求。
    // （可选本地素材包由构建环境决定是否打进 lib，因此这里不断言“设备区为空”。）
    for (const model of ['iPhone 14 Pro', 'MacBook Pro (2018)', 'Pro Display XDR', 'Apple Watch Ultra']) {
        await expect(drawer.getByText(model, { exact: true })).toHaveCount(0);
    }
    expect(requests.filter((url) => /devicescss/.test(url))).toEqual([]);
    const browserFrames = drawer.locator('.shoteasy-frame-option').filter({ hasText: '简洁浏览器' });
    await expect(browserFrames.first()).toBeVisible();
});

test('saved Devices.css projects preserve their frame and block incomplete library exports', async ({ page }) => {
    await page.addInitScript(() => { window.showSaveFilePicker = undefined; });
    const deviceRequests = [];
    page.on('request', request => { if (/devicescss.*\.png/.test(request.url())) deviceRequests.push(request.url()); });
    await page.goto('/?workspace=true');
    const first = page.locator('#consumer-a');
    await first.locator('.shoteasy-upload-card input[type=file]').setInputFiles({
        name: 'missing-device.png', mimeType: 'image/png', buffer: createPngFixture(64, 48),
    });
    const saveProject = async () => {
        const download = page.waitForEvent('download');
        await first.getByRole('menuitem', { name: '文件', exact: true }).click();
        await page.getByRole('menuitem', { name: /^保存项目/ }).click();
        return unzipSync(await readDownload(await download));
    };
    const entries = await saveProject();
    await expect(first.getByRole('button', { name: '导出图片', exact: true })).toBeEnabled();
    const manifest = JSON.parse(strFromU8(entries['manifest.json']));
    manifest.name = 'Missing device regression';
    manifest.document.option.frame = 'devicescss-apple-watch-ultra-v1';
    entries['manifest.json'] = strToU8(JSON.stringify(manifest));
    await first.getByTestId('project-file-input').setInputFiles({
        name: 'missing-device.screenhello', mimeType: 'application/vnd.screenhello.project+zip', buffer: Buffer.from(zipSync(entries)),
    });
    await expect(first.getByRole('button', { name: /项目：Missing device regression/ })).toBeVisible();
    await expect(first.getByRole('button', { name: '导出图片', exact: true })).toBeEnabled();
    const exportedImages = [];
    const onDownload = download => exportedImages.push(download.suggestedFilename());
    page.on('download', onDownload);
    await first.getByRole('menuitem', { name: '文件', exact: true }).click();
    await page.getByRole('menuitem', { name: /使用当前设置快速导出/ }).click();
    await expect(page.getByText('设备素材缺失或渲染失败，请重新选择设备或改用无外框后导出。', { exact: true })).toBeVisible();
    expect(exportedImages).toEqual([]);
    page.off('download', onDownload);
    const roundTrip = JSON.parse(strFromU8((await saveProject())['manifest.json']));
    expect(roundTrip.document.option.frame).toBe(manifest.document.option.frame);
