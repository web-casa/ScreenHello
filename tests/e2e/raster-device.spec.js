...
test('device model cards group colors, keyboard changes undo once, seven locales and legacy appearance stay intact', async ({ page }, testInfo) => {
    ...
    // 设备卡片数随素材包变化（本地包 + 提交进仓库的 Devices.css 机型），从应用自身取真值
    const deviceModels = await page.evaluate(async () => (await import('/src/utils/frameConfig.js'))
        .getFrameGroups().find(group => group.id === 'device').items.length);
    await expect(group.locator('.shoteasy-frame-option')).toHaveCount(deviceModels);
...
test('real device choices exclude legacy vector frames and retain source links without verification warnings', async ({ page }, testInfo) => {
    ...
    page.on('console', message => {
        if (message.type() === 'error' && /Cannot update a component \(`Notification`\) while rendering/.test(message.text())) {
    ...
    // 精确匹配历史素材包的「iPhone」卡片：Devices.css 也带来了 iPhone 14 / X 等型号
    ...
    expect(await page.evaluate(() => window.__deviceOperation)).toBe(true);
    // Ant Design's former three-second duration path used rAF updates. Wait past
    // that boundary so a notification timer cannot regress into a render-phase
    // state update after the export command has settled.
