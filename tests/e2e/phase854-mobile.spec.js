...
test('[Phase 8.5.4] phone shell has one menu, project status, and a fixed export action', async ({ browser, baseURL }, testInfo) => {
    ...
    try {
        ...
        const privacyBadge = topbar.locator('.shoteasy-privacy-badge');
        ...
        await expect(privacyBadge).toBeHidden();
        await expect(exportAction).toBeVisible();
        expect(await topbar.getByRole('button').allTextContents()).toHaveLength(5);

        for (const target of [mobileMenu, projectStatus, exportAction, topbar.getByRole('button', { name: '切换主题' }), topbar.getByRole('button', { name: '语言' })]) {
        ...
        await expect(drawer.getByRole('menuitem', { name: /本地数据与隐私/ })).toBeVisible();
