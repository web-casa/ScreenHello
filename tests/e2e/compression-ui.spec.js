...
test('AVIF standard-only controls and actual download preserve format and dimensions', async ({ page }) => {
    ...
    for (const locale of ['zh-CN', 'en-US', 'zh-TW', 'de-DE', 'ko-KR', 'es-ES', 'pt-PT']) {
        ...
        await expect(page.getByTestId('avif-standard-only')).toContainText('AVIF');
        await expect(page.getByTestId('compression-standard')).toHaveAttribute('aria-pressed', 'true');
        await expect(page.getByTestId('compression-lossy')).toHaveCount(0);
    ...
    const { decodeAvifFile } = await import('../release/avif-file-decoder.mjs');
    expect(await decodeAvifFile(bytes.toString('base64'))).toMatchObject({ width: 73, height: 55 });
    expect(await page.evaluate(() => window.__shoteasyStores.workspace.exportSettings)).toEqual({ format: 'avif', ratio: 1 });
    await page.getByRole('button', { name: '导出图片', exact: true }).click();
    await drawer.getByText('PNG', { exact: true }).click();
    await preview(page);
});
...
test('legacy AVIF quick export uses standard settings without changing scale', async ({ page }) => {
    await openEditor(page);
    await page.getByTestId('export-cancel').click();
    const result = await page.evaluate(async () => {
        const root = window.__shoteasyStores;
        const original = root.exportService.downloadImage;
        let request;
        root.exportService.downloadImage = async value => { request = value; return { current: true }; };
        const legacy = { format: 'avif', ratio: 2, compression: 'lossy', quality: 40 };
        root.workspace.setExportSettings(legacy, { replace: true });
        try {
            const ok = await root.commands.downloadCurrentImage(legacy);
            return { ok, format: request.format, ratio: request.ratio, compression: request.compression ?? null,
                quality: request.quality ?? null, saved: root.workspace.exportSettings };
        } finally { root.exportService.downloadImage = original; }
    });
    expect(result).toEqual({ ok: true, format: 'avif', ratio: 2, compression: null, quality: null, saved: { format: 'avif', ratio: 2 } });
});
