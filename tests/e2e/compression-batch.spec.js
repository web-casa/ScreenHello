...
test('legacy AVIF batch settings become standard exports without changing scale', async ({ page }) => {
    await openEditor(page);
    const result = await page.evaluate(async bytes => {
        const root = window.__shoteasyStores;
        root.workspace.setExportSettings({ format: 'avif', compression: 'lossy', quality: 80, ratio: 2 }, { replace: true });
        await root.batch.start([new File([new Uint8Array(bytes)], 'only.png', { type: 'image/png' })]);
        return { settings: root.workspace.exportSettings, frozen: root.batch.snapshotSettings,
            statuses: root.batch.jobs.map(job => job.status), archive: Boolean(root.batch.archive), leases: root.exportService._canvasLeases.size };
    }, [...createPngFixture(64, 48)]);
    expect(result).toEqual({ settings: { format: 'avif', ratio: 2 }, frozen: { format: 'avif', ratio: 2 },
        statuses: ['completed'], archive: true, leases: 0 });
});

test('legacy AVIF project and preset application preserve scene and scale while selecting standard export', async ({ page }) => {
    await openEditor(page);
    const original = await page.evaluate(async () => [...new Uint8Array(await (await window.__shoteasyStores.workspace.createProjectBlob()).arrayBuffer())]);
    const entries = unzipSync(new Uint8Array(original));
    const manifest = JSON.parse(strFromU8(entries['manifest.json']));
    manifest.exportSettings = { format: 'avif', ratio: 2, compression: 'lossy', quality: 40 };
    entries['manifest.json'] = strToU8(JSON.stringify(manifest));
    const result = await page.evaluate(async bytes => {
        const root = window.__shoteasyStores;
        const file = new File([new Uint8Array(bytes)], 'legacy.screenhello');
        const ok = await root.workspace.openProjectFile(file);
        const opened = { ...root.workspace.exportSettings };
        const images = root.imageStore.list.map(image => ({ width: image.width, height: image.height }));
        await root.workspace.savePreset('Legacy AVIF');
        const id = root.workspace.presets.find(item => item.name === 'Legacy AVIF').id;
        const record = await root.draftStore.loadPreset(id);
        record.preset.exportSettings = { format: 'avif', ratio: 3, compression: 'lossy', quality: 80 };
        ...
        return { ok, opened, images, applied, current: root.workspace.exportSettings,
            original: (await readWorkspaceArchive(file)).exportSettings };
    }, [...zipSync(entries)]);
    expect(result).toMatchObject({ ok: true, opened: { format: 'avif', ratio: 2 }, applied: true,
        current: { format: 'avif', ratio: 3 }, original: manifest.exportSettings });
