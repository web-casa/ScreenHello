...
test('language menu is operable entirely by keyboard and announces the selection', async ({ page }) => {
    ...
    // 标签现在跟随界面语言（原来是无条件英文 Light/Dark），因此按"括号内是本地化状态"断言，
    // 并单独验证它会随主题切换而变化（见下面的 Dark 一条）。
...
for (const width of [320, 390, 1280]) {
    test(`top-right appearance controls support seven languages and fit at ${width}px`, async ({ page }, testInfo) => {
        ...
        // StandaloneLocale must persist the visible document language before a
        // reload can observe it. This catches a passive-effect persistence race.
        await expect.poll(() => page.evaluate(() => localStorage.getItem('SCREENHELLO_LOCALE'))).toBe('pt-PT');
        await page.reload();
