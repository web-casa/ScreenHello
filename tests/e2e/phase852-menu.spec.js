...
async function expectViewportOverlay(page, locator, { fixed = false } = {}) {
...
test('[Phase 8.5.2] desktop overlays use viewport layout and browser URL remains editable', async ({ page }) => {
    await openOffline(page);
    await importFixture(page);

    const file = page.getByRole('menubar', { name: '应用菜单' }).getByRole('menuitem', { name: '文件' });
    await file.click();
    await expectViewportOverlay(page, page.locator('.shoteasy-command-menu--file'));
    await page.keyboard.press('Escape');

    await page.getByRole('button', { name: '选择画布尺寸' }).click();
    await expectViewportOverlay(page, page.locator('.shoteasy-size-overlay'));
    await page.keyboard.press('Escape');

    await page.locator('.shoteasy-frame-option input[value="macosBarLight"]').check({ force: true });
    const browserUrl = page.getByRole('textbox', { name: '浏览器地址栏 URL' });
    await expect(browserUrl).toBeVisible();
    await expect(browserUrl).toBeEditable();
    await browserUrl.fill('https://example.com/desktop-preview');
    await expect(browserUrl).toHaveValue('https://example.com/desktop-preview');

    await page.getByRole('button', { name: '裁剪图片' }).click();
    await expectViewportOverlay(page, page.locator('.shoteasy-cropper-modal .ant-modal-wrap'), { fixed: true });
    await page.getByRole('dialog', { name: '裁剪' }).getByRole('button', { name: /取\s*消/ }).click();

    await page.getByRole('button', { name: '导出图片' }).click();
    await expectViewportOverlay(page, page.locator('.shoteasy-export-overlay.ant-drawer'), { fixed: true });
    await expectViewportOverlay(page, page.locator('.shoteasy-export-overlay .ant-drawer-content-wrapper'));
...
test('[Phase 8.5.2] view and help commands change only local presentation state', async ({ page }) => {
    ...
    await page.evaluate(() => {
        ...
        const recordExternalLink = (event) => {
            const anchor = event.target instanceof HTMLAnchorElement
                ? event.target : event.target?.closest?.('a');
            if (!anchor || anchor.href !== 'https://github.com/web-casa/ScreenHello') return;
            // Capture the browser-standard click event rather than replacing
            // HTMLElement.prototype.click: WebKit can invoke that method
            // natively without calling a JavaScript prototype replacement.
            event.preventDefault();
            globalThis.__screenhelloExternalLink = { href: anchor.href, target: anchor.target, rel: anchor.rel };
            document.removeEventListener('click', recordExternalLink, true);
        };
        document.addEventListener('click', recordExternalLink, true);
    });
    await help.click();
    await page.getByRole('menuitem', { name: /GitHub 仓库/ }).click();
    await expect.poll(() => page.evaluate(() => globalThis.__screenhelloExternalLink)).toEqual({
