...
const fixture = () => ({
    ...
    avifPreviewCapability: { state: 'standard-only', noticeVisible: true, previewEnabled: false, directDownloadEnabled: true },
    ...
    largeAvifStandardOnly: { lossyDisabled: true, standardEnabled: true, previewDisabled: true, noDownload: true },
});
...
describe('bounded browser compression evidence', () => {
    ...
    it('accepts the eight exact cases and explicit large-AVIF standard-only controls', () => {
        expect(() => validateCompressionEvidence(fixture())).not.toThrow();
        expect(compressionCases()).toHaveLength(8);
    });
    it.each([
        ['missing case', e => e.results.pop()],
        ['old v2 scope', e => { e.scope = 'foreground-compression-download/v2'; }],
        ...
        ['removed preview unexpectedly available', e => { e.avifPreviewCapability.previewEnabled = true; }],
        ...
        ['large AVIF enabled', e => { e.largeAvifStandardOnly.lossyDisabled = false; }],
        ['unexpected control-check download', e => { e.largeAvifStandardOnly.noDownload = false; }],
