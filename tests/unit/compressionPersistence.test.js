...
import { exportSettingsWarnings, normalizeExportSettings, normalizeEditorExportSettings, validateExportSettings } from '../../src/utils/exportSettings.js';
...
describe('AVIF standard-only editor settings', () => {
    it.each([1, 2, 3])('migrates legacy AVIF quality without mutating archives or scale %s', ratio => {
        const legacy = Object.freeze({ format: 'avif', ratio, compression: 'lossy', quality: 40 });
        expect(normalizeEditorExportSettings(legacy)).toEqual({ format: 'avif', ratio });
        expect(normalizeExportSettings(legacy)).toEqual(legacy);
        expect(validateExportSettings(legacy)).toEqual(legacy);
    });
    it.each([
        { format: 'png', ratio: 2, compression: 'lossy', paletteColors: 64 },
        { format: 'png', ratio: 1, compression: 'lossless' },
        { format: 'webp', ratio: 3, compression: 'lossless' },
        { format: 'webp', ratio: 1, compression: 'lossy', quality: 60 },
        { format: 'jpg', ratio: 2, compression: 'lossy', quality: 80 },
    ])('preserves other editor modes %j', settings => {
        expect(normalizeEditorExportSettings(settings)).toEqual(settings);
    });
});
