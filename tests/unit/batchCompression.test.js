...
describe('C3 frozen batch compression', () => {
    it.each([false, true])('normalizes legacy AVIF batch snapshots and retries (preset=%s)', async preset => {
        const settings = { format: 'avif', ratio: 2, compression: 'lossy', quality: 40 };
        const h = harness({ settings });
        h.root.exportService.exportImage.mockRejectedValueOnce(Object.assign(new Error('cancelled'), { code: 'export-cancelled' }));
        await h.store.start([file('one.png')], { presetId: preset ? 'legacy' : null });
        expect(h.store.snapshotSettings).toEqual({ format: 'avif', ratio: 2 });
        await h.store.retryFailed();
        for (const [request] of h.root.exportService.exportImage.mock.calls) {
            expect(request).toMatchObject({ format: 'avif', ratio: 2 });
            expect(request.compression).toBeUndefined();
