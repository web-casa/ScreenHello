...
describe('CommandService', () => {
    it('quick export migrates restored AVIF settings to the standard path', async () => {
        ...
        const download = vi.spyOn(runtime.exportService, 'downloadImage').mockResolvedValue({ current: true });
        expect(await runtime.commands.downloadCurrentImage()).toBe(true);
        expect(download.mock.calls[0][0]).toMatchObject({ format: 'avif', ratio: 1 });
        expect(download.mock.calls[0][0].compression).toBeUndefined();
        expect(download.mock.calls[0][0].quality).toBeUndefined();
        expect(runtime.workspace.exportSettings).toEqual({ format: 'avif', ratio: 1 });
        expect(runtime.commands.exportActive).toBe(false);
    });
    it('defers browser export notices and replaces queued loading with the final result', async () => {
        const runtime = createRuntime();
        const open = vi.fn();
        const timers = [];
        runtime.editor.setMessage({ open });
        vi.spyOn(runtime.editor, 'ensureEditing').mockReturnValue(true);
        ...
        await expect(runtime.commands.downloadCurrentImage()).resolves.toBe(true);
        expect(open).not.toHaveBeenCalled();
        expect(window.clearTimeout).toHaveBeenCalledWith(1);

        timers[1].callback();
        expect(open).toHaveBeenCalledOnce();
        expect(open).toHaveBeenCalledWith(expect.objectContaining({ type: 'success', content: '下载成功' }));
    });
