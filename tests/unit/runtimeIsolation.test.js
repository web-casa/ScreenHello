...
import { privacyInstrumentationState } from '../../src/utils/privacyMonitor.js';
...
describe('ScreenHelloRuntime isolation', () => {
    it('does not subscribe discarded constructors and releases all activated instances', () => {
        const original = globalThis.fetch;
        createRuntime(); // React can discard a lazy initializer without running an effect cleanup.
        expect(privacyInstrumentationState()).toMatchObject({ listeners: 0, patched: false });
        const first = createRuntime(), second = createRuntime();
        first.activate({ onlyIfNone: true });
        first.activate({ onlyIfNone: true });
        second.activate({ onlyIfNone: true });
        expect(privacyInstrumentationState().listeners).toBe(2);
