...
test('production SEO: all 42 pages have readable HTML, self canonicals and language alternatives', async ({ request, baseURL }) => {
    ...
    for (const slug of SITE_LANGUAGES) for (const topic of SITE_TOPICS) {
        const route = `docs/${slug}/${topic ? `${topic}/` : ''}`;
        ...
        expect(html).toContain(`"dateModified":"${catalog[slug].reviewed}"`);
        expect(html).not.toMatch(/\/src\/main|googletagmanager/);
    }
    const sitemap = await request.get(new URL('sitemap.xml', baseURL).href);
    expect(sitemap.headers()['content-type']).toContain('xml');
    const sitemapText = await sitemap.text();
    expect(sitemapText.match(/<loc>/g)).toHaveLength(43);
    expect(sitemapText.match(/<lastmod>/g)).toHaveLength(43);
    ...
    expect(redirect.headers().location).toBe(`${base}docs/en/guide/`);
...
test.describe('content without JavaScript', () => {
    ...
    for (const slug of SITE_LANGUAGES) {
        test(`production SEO: ${slug} content and language navigation work without JavaScript`, async ({ page, baseURL }) => {
            ...
            await english.click();
            await expect(page).toHaveURL(new URL('docs/en/', baseURL).href);
            expect(scripts).toEqual([]);
        });
    }
});

test.describe('static system theme', () => {
    test.use({ javaScriptEnabled: false, colorScheme: 'dark' });

    test('production SEO: docs expose readable dark tokens before client hydration', async ({ page }) => {
        ...
        })).toEqual({ background: '#111318', foreground: '#e9ecf1', card: '#171b22' });
    });
});
