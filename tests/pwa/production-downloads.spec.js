...
const modes = [
    ...
    ['pc', 'avif', 'standard'], ['small', 'avif', 'standard'], ['small', 'png', 'lossless', 2],
];
...
for (const [fixture, format, compression, ratio = 1] of modes) {
    test(`frozen production ${fixture} ${format}/${compression} ${ratio}x downloads decodable exact dimensions`, async ({ page }, testInfo) => {
        ...
        if (fixture === 'pc') {
            ...
            if (format === 'avif') await expect(page.getByTestId('export-preview')).toHaveCount(0);
            else await expect(page.getByTestId('export-preview')).toBeDisabled();
...
test('frozen production offers only standard AVIF including PC-size exports', async ({ page }) => {
    await page.goto('/');
