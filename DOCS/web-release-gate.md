...
> 更新日期：2026-09-05。Phase 8.5 第七候选 `76035d8004d556771ce327e234811cee94313917` 的技术 Gate 已通过：公共 CI `33867398243` attempt 2、四浏览器矩阵 `33867740923` 和同 SHA schema v2 汇总均通过。此前第六轮 HOLD 已关闭；Draft PR #5 尚未合并，未正式发布。后续源码（包括桌面阶段及保存/多实例修复）不得自动继承该 SHA 的最低浏览器通过结论。

## 0. Phase 8.5 候选历史与最终通过证据

最终第七候选的 [公共 CI](https://github.com/web-casa/ScreenHello/actions/runs/33867398243) 与 [Chrome/Edge/Firefox/Safari 矩阵](https://github.com/web-casa/ScreenHello/actions/runs/33867740923) 均已通过。下列逐轮记录保留排障经过，不是当前待执行任务；其中本地数字属于各自采集时点，当前工程摘要见 [质量基线](./quality-baseline.md)。
...
公开仓 `main` 仍是 Phase 7 的 `4d318fa9ea8961faf148d22720458b7e8b4af7eb`；Phase 8.5 候选位于 Draft PR #5 的 `phase-8.5-web-ux` 分支。第七修复已完成同 SHA CI、四浏览器复跑和下载后 schema v2 汇总，未混用前六轮单项成功证据。技术通过、合并 main 与创建正式发布是不同状态。
...
Phase 8.5 指定候选的本地、公共 CI 与 schema v2 浏览器矩阵均已 GO；不能将第六轮 HOLD 写成最终状态，也不能把旧 SHA 的 GO 套到后续修改。当前没有正式 main 合并、tag/Release、Web 部署或 npm 发布；后续候选需要自己的验收记录及相应外部动作授权。
