...
- Web-only 七语静态产品/美化/外框/压缩/指南/隐私页面（42 页），由独立的 Fumadocs 文档站（`docs/`）生成在 `/docs/{locale}/{topic}/`；旧的 `/{locale}/{topic}/` 全部 301 到新地址。保留根编辑器及本地草稿，PWA 导航回退仅限编辑器根入口。静态内容页不运行编辑器，也不属于首次离线缓存。实现、CSP hash 与上线边界见 [SEO / GEO](./seo-geo.md)。
...
- 顶栏隐私计数按页面级运行时出口监控显示已知外发字节、外部请求和仅接收的外部资源加载；流式或 multipart 请求体显示已知字节下界与无法确定提示，WebSocket 同时统计连接和成功发送。
