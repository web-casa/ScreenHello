...
- 独立站项目中心支持 `.screenhello` 项目打开/保存/另存为、最多 12 个最近项目、草稿管理和存储状态；组件库用 `workspace` 显式开启。
- 完整风格预设支持本地保存、应用、复制、重命名、删除和 `.screenhello-preset` 导入/导出。
- 图片边缘色、内描边和横竖图外框建议完全在本地生成，只在用户点击后应用。
- 同一页面可挂载多个互相隔离的编辑器；图片、选项、历史、主题、草稿服务、Leafer App 和运行时资源按实例拥有。
- ProjectDocument v2 多图片画布；图层选择/多选、移动/缩放/旋转、复制/删除、编组/锁定、层序、对齐/吸附/等间距及堆叠/扇形布局。
...
- 自动化测试：已接入 Vitest 单元测试、Playwright 当前三引擎 smoke/E2E、Chromium visual/export golden 和 library consumer smoke；完整覆盖仍在后续阶段扩充。
- 服务端能力：没有 API、数据库、账户系统或上传服务。
- Web Release Gate 已通过：同一候选提交在原生 amd64 Chrome 111、Edge 111、Firefox 128 与 GitHub `macos-14` Safari 26.5 上完成核心编辑和四格式导出。产品仍声明 Safari 16.4+，但当前测试策略不精确重放历史 16.4。首次公开的本地整理正在进行，正式公共仓写入、tag/release 与部署仍需单独授权。详见 [Web Release Gate](./web-release-gate.md)。
...
│  │  └─ workspace/              项目、预设、草稿、建议与存储状态 UI
│  ├─ hooks/                     图片载入、粘贴、导出快捷键
│  ├─ platform/                  浏览器文件、存储、剪贴板、捕获与导出能力边界
...
- 项目文件增强保存依赖 File System Access；能力不存在或未授权时自动退回文件选择和下载。
