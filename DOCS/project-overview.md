...
2026-09-17 新增 [MAS / Microsoft Store 双架构打包入口](./desktop-store-packaging.md)，当前为未在原生系统运行的开发入口，尚缺商店身份、MAS 证书/profile 和实机验收。六目标直装已发起公开 Actions；Linux 候选基线调整为 Ubuntu 24.04，旧系统不再列为本轮支持目标。

2026-09-17 当前工作树支持 [iPhone Duo 两种非手持本地样式](./iphone-duo.md)，手持款已按用户要求撤下；素材公开再分发尚未确认，不包含在此前生成的测试 DMG 中。

2026-09-17 项目审阅发现的四项异步生命周期问题已修复：保存快照基线、原生退出确认、预设请求失效与选择器 token 回收。另修复打开项目缓存和保存预设时的同类快照问题；验证及平台边界见[项目审阅记录](./project-review-2026-09-17.md)。现已在公开仓库生成[新的签名、公证 ARM64 DMG](./macos-dmg-test-build-2026-09-17.md)，用户 Mac 上的安装与完整 GUI 验收仍未执行。

2026-09-17 追加[桌面 review 修复](./desktop-review-fixes-2026-09-17.md)：截图窗口恢复、文件名处理和权限/跨端契约测试；已包含在上述测试 DMG 中。
