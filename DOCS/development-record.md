...
同日追加 iPhone Duo 本地开发：提取三份 PSD 的四种样式，增加屏幕上方的手部／摄像头前景、缺包保护、七语名称和不携带第三方素材的项目存档。开发过程与验证见 [iPhone Duo](./iphone-duo.md)；本次没有发布新 DMG。随后按用户要求撤下两款手持样式，选择器与构建仅保留非手持横屏／竖屏，旧手持 ID 保留为不可用以避免静默改变存档。
...
2026-09-17 用户授权构建新的公开 macOS ARM64 测试包。候选 `1ac61bf` 在 run `35132257556` 第二次尝试成功，app/DMG 签名、公证、staple 和 Gatekeeper 通过，下载后的 SHA-256 已复核。第一次尝试在 app 公证通过后遇到 DMG 脚本错误；独立 Linux CI 和通用桌面 Gate 的失败仍保留，没有合并 PR 或创建 Release。详见[DMG 交付记录](./macos-dmg-test-build-2026-09-17.md)。
