...
安全重试由边界外的实例级恢复上下文传递，不读取/写入/清空原草稿，也跳过默认图；普通重试保持原行为。自动恢复尚未完成时禁止保存抢写，损坏/无法读取的草稿会触发本会话保护，避免新导入图片覆盖待恢复记录。

`src/i18n/` 提供实例级 I18nStore、纯文本词典与订阅 Hook。语言变化只更新 UI；项目文档、历史和用户输入不变，不调用 MobX 全局 configure。只有独立站入口同步 HTML lang/title 和本地语言偏好，library 不修改宿主文档语言。
...
开始任务时，BatchStore 同步捕获当前风格，或记录所选本地预设 ID 供 service 读取；两者都在惰性加载前捕获影响画框描边的主题，仅用于本批次，不写入存档。C3 在背景解析/隔离 renderer 初始化成功后，向 Store 交付已深冻结的 option/完整 exportSettings、主题与不可变背景 Blob；Store 以 observableRef 持有，重试复用同一快照，不重读预设或 URL，新批次/选新文件/clear/dispose 才释放引用。解析失败不生成可重试快照，详情见 [C3](./export-compression-c3.md)。

service 只维护 1～12 个串行 job；每项在独立的隐藏 Leafer App 中复用 `FrameBox`、`Screenshot` 和 `Watermark`，经所属 runtime 的 ExportService `target + size` 内部契约编码，再由 fflate pass-through 写入一个 ZIP。显式 target 的就绪由隔离 renderer 管理，不借用活动画布内容版本；共享串行队列和实例销毁保护不变。隔离 runtime 不启用 BatchStore、workspace、草稿、历史或交互编辑器，结束时释放图片 object URL、背景资源、Canvas 和 DOM。
...
C1 在普通和隔离 runtime 中统一提供 `RenderTaskTracker`：项目内容版本与节点绘制版本分开，效果完成属于同内容的收敛，用户编辑则使请求失效。底图服务与最终导出共用捕获互斥，避免临时隐藏标注污染最终像素；编码和系统保存不持有此互斥。新压缩路径只捕获一次完整树，从该快照生成标准参照及压缩结果。内部预览 token 由 ExportService 持有，交付成功后消费，当前有效的系统取消/失败结果可重试；UI 偏好由命令层在成功后提交，详情见 [C1](./export-compression-c1.md)。

C2 由导出抽屉的 `useExportPreview` 持有临时 UI 会话；解码验证纳入导出队列，`ExportPreviewResult` 切侧时释放旧图片与 URL、保留 100% 滚动位置，不重复编码。预览解码/资源不写入项目或 IndexedDB，不共享业务单例。参数变化立即清除会话，关闭/卸载通过代际拒绝晚到结果；旧 owner 清理不能消费另一 token。平台交付与展示 URL 分离，具体边界见 [C2](./export-compression-c2.md)。
