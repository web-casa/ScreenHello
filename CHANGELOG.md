...
- 项目保存期间继续编辑时，新修改保持“未保存”；打开项目后的缓存写入不再覆盖新名称或清除新修改，风格预设保存使用同一时刻的配置快照。
- 桌面关闭窗口和应用退出统一检查未保存内容，支持保存、不保存、取消；保存失败或仍有新修改时保留编辑器。
- 连续选择预设或切换项目时忽略过期的异步结果；文件选择器在运行时销毁后返回的句柄仍会释放。

- 桌面截图避免无条件唤起原先隐藏或最小化的窗口，按开始时的焦点状态决定是否显式请求焦点；保存对话框建议名称兼容 Windows/Unix 路径分隔符。
