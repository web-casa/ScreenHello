...
const desktopCargoVersion = cargoToml.match(/^version\s*=\s*"(?<version>[^"]+)"\s*$/mu)?.groups?.version;
const desktopLockVersion = cargoLock.match(/\[\[package\]\]\s*\nname = "screenhello-desktop"\s*\nversion = "(?<version>[^"]+)"/u)?.groups?.version;
expectEqual(config.version, packageJson.version, 'desktop-tauri-version-source-mismatch');
...
const expectedPermissions = [
    ...
    'allow-desktop-state-status',
    'allow-desktop-set-locale',
    ...
    'allow-desktop-capture-capability',
...
expectEqual(config.bundle?.macOS?.minimumSystemVersion, '11.0', 'desktop-macos-minimum-system-version-invalid');
expectEqual(config.bundle?.windows, {
    allowDowngrades: false,
    nsis: { installMode: 'currentUser' },
}, 'desktop-windows-install-lifecycle-config-invalid');
if (JSON.stringify(config.bundle?.windows?.nsis || {}).includes('installerHooks')) {
    failures.push('desktop-windows-uninstall-hook-forbidden');
}
...
const cspDirectiveIncludes = (policy, directive, source) => policy
    .split(';')
    .map((value) => value.trim().split(/\s+/u))
    .some(([name, ...sources]) => name === directive && sources.includes(source));
const auditCsp = (value, allowedOrigins, id) => {
    ...
    if (!cspDirectiveIncludes(policy, 'script-src', "'self'")) failures.push(`${id}-script-self-missing`);
    if (!cspDirectiveIncludes(policy, 'script-src', "'wasm-unsafe-eval'")) failures.push(`${id}-wasm-eval-missing`);
    if (cspDirectiveIncludes(policy, 'script-src', "'unsafe-eval'")) failures.push(`${id}-unsafe-eval-forbidden`);
...
const captureCapabilityBlock = captureSource.match(/struct CaptureCapability \{(?<body>[\s\S]*?)\n\}/u)?.groups?.body || '';
if (!captureCapabilityBlock || /environment|display|session|hostname|\bpath\b/u.test(captureCapabilityBlock)) {
    failures.push('desktop-capture-capability-exposes-native-detail');
}
const desktopStateStatusBlock = desktopStateSource.match(/struct DesktopStateStatus \{(?<body>[\s\S]*?)\n\}/u)?.groups?.body || '';
if (!desktopStateStatusBlock || /environment|hostname|\bpath\b|last_seen_app_version|error/u.test(desktopStateStatusBlock)) {
    failures.push('desktop-state-response-exposes-storage-detail');
}
for (const required of [
    ...
    'fn reconcile_marker(',
    'fn desktop_state_status(',
]) {
    if (!desktopStateSource.includes(required)) failures.push(`desktop-state-boundary-missing:${required}`);
}
if (/remove_(?:file|dir|dir_all)/u.test(desktopStateSource)) {
    failures.push('desktop-state-data-deletion-forbidden');
}
for (const required of [
    ...
    'desktop-capture-system-permission-required',
    'desktop-capture-wayland-portal-required',
    'CGRequestScreenCaptureAccess',
    'fn desktop_capture_capability()',
    'Response::new(bytes)',
]) {
...
for (const dependency of [
    ...
    'objc2-core-graphics = { version = "=0.3.2", default-features = false, features = ["CGWindow"] }',
]) {
...
if (!/#\[cfg\(feature = "desktop-test-driver"\)\][\s\S]{0,700}tauri_plugin_wdio_webdriver::init\(\)/u.test(rustSource)) {
    failures.push('desktop-test-driver-registration-not-feature-gated');
}
for (const marker of ['SCREENHELLO_TEST_DRIVER_RUN', 'SCREENHELLO_RUNNER_ONLY_TEST_BINARY']) {
    if (!rustSource.includes(marker)) failures.push(`desktop-driver-runtime-gate-missing:${marker}`);
}
if (!buildScript.includes('CARGO_FEATURE_DESKTOP_TEST_DRIVER') || !buildScript.includes('SCREENHELLO_TEST_DRIVER_BUILD')) {
    failures.push('desktop-driver-build-gate-missing');
}
expectEqual(packageJson.scripts?.['desktop:build:test-driver'], 'node scripts/build-desktop-test-driver.mjs', 'desktop-test-build-entry-invalid');
