...
import { validateExportSettings, normalizeEditorExportSettings, WEB_AVIF_LIMIT_MESSAGE } from '../utils/exportSettings';
...
export class CommandService {
    ...
    constructor(root) {
        ...
        this._exportNoticeTimers = new Map();
    ...
    deleteSelection({ imagesOnly = false } = {}) {
        ...
        // removeSelected 只删未锁定图层，提示数量必须用同一条件统计，否则会多报
        const removedImageCount = this.root.imageStore.selectedList.filter((layer) => !layer.locked).length;
        ...
        editorTarget?.cancel?.();
        if (removedImages || removedShapeCount) {
            // 不传 mergeKey：每次删除各自成一步，toast 里「已删除 N 个…」与一次撤销严格对应
            this.root.history.commit();
            this._announceDeleteUndo(removedImageCount, removedShapeCount);
        }
        if (removedImages && !this.root.imageStore.list.length) this.root.editor.clearFun?.();
        return removedImages || removedShapeCount > 0;
    }

    /**
     * 删图层/标注是高频且易恢复的操作：不弹确认框，直接执行后用 toast 提供一次撤销。
     * 撤销凭据只在这仍是栈顶时有效，因此删除后又有新编辑时不会再误撤销。
     * 固定 key 让连续删除的提示互相替换，不会留下一个点不动的旧提示。
     */
    _announceDeleteUndo(imageCount, shapeCount) {
        if (!imageCount && !shapeCount) return;
        const label = imageCount && shapeCount
            ? this.root.i18n.t('已删除 {0} 个图层和 {1} 个标注', { 0: imageCount, 1: shapeCount })
            : shapeCount
                ? this.root.i18n.t('已删除 {0} 个标注', { 0: shapeCount })
                : this.root.i18n.t('已删除 {0} 个图层', { 0: imageCount });
        const token = this.root.history.captureUndoToken();
        showUndoToast(this.root, {
            label,
            key: 'undo:selection-delete',
            onUndo: () => this.root.history.undoTo(token),
            onUnavailable: () => this.root.editor.message?.info?.(this.root.i18n.t('没有可撤销的操作')),
        });
    }
    ...
    /**
     * Ant Design 的 message API 会立即更新 context holder。导出开始时刚写入
     * `exportActive`，MobX observer 可能仍在重绘侧栏；把该次写入放到下一个任务，
     * 避免 Notification 在 FrameBar render 中更新。相同 key 的未执行 loading
     * 提示会由最终 success/error 替换，因而快速导出不会留下过期状态。
     */
    _showExportMessage(config) {
        const message = this.root.editor.message;
        if (typeof message?.open !== 'function') return;
        const browser = globalThis.window;
        if (typeof browser?.setTimeout !== 'function') {
            message.open(config);
            return;
        }
        const timerKey = config.key ?? Symbol('export-message');
        const previousTimer = this._exportNoticeTimers.get(timerKey);
        if (previousTimer !== undefined) browser.clearTimeout?.(previousTimer);
        const timer = browser.setTimeout(() => {
            this._exportNoticeTimers.delete(timerKey);
            if (this.root.isDisposed || this.root.editor.message !== message) return;
            message.open(config);
        }, 0);
        this._exportNoticeTimers.set(timerKey, timer);
    }
    ...
    async _runExport(kind, confirmedSettings, preparedToken) {
        ...
        let settings;
        try { settings = normalizeEditorExportSettings(validateExportSettings(confirmedSettings ?? this.root.workspace.exportSettings)); }
        ...
        this._showExportMessage({ key, type: 'loading', content: kind === 'copy' ? this.root.i18n.t("正在复制…") : this.root.i18n.t("正在下载…") });
        try {
            ...
            if (result?.summary?.warnings?.length) {
                this._showExportMessage({ type: 'warning', content: this.root.i18n.t('部分增强效果不可用，本次按画面中的回退效果导出。') });
            }
            if (!this.root.isDisposed) {
                this._showExportMessage({ key, type: 'success', content: kind === 'copy' ? this.root.i18n.t("复制成功") : this.root.i18n.t("下载成功") });
            }
            return true;
        } catch (error) {
            if (!controller.signal.aborted && !this.root.isDisposed && !isExportCancelled(error)) {
                this._showExportMessage({ key, type: 'error', content: exportFailureMessage(error, action, format, this.root.i18n.t) });
            }
            return false;
        } finally {
    ...
    dispose() {
        this.consumePageUnloadApproval();
        const browser = globalThis.window;
        for (const timer of this._exportNoticeTimers.values()) browser?.clearTimeout?.(timer);
        this._exportNoticeTimers.clear();
        this._exportController?.abort();
        this._exportController = null;
