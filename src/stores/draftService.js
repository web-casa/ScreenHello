...
export class DraftService {
    ...
    _protectUnrestoredDraft() {
        this.recoveryBlocked = true;
        this._autosaveEnabled = false;
        this.status = 'error';
        this.errorCode = 'draft-recovery-blocked';
        clearTimeout(this._saveTimer);
        this._saveTimer = null;
        this.root.editor.setClearDraftHook(null);
    }
    ...
    async _save({ key, generation }) {
        ...
        await this.root.draftStore.saveProject(key, doc, {
            kind: 'draft',
            name: this.root.editor.img?.name || this.root.i18n.t("自动草稿"),
        });
    ...
    _handleSaveError(err) {
        ...
        if (code === 'image-blob-unavailable') {
            ...
            if (!repeated) this.root.editor.message?.warning?.(this.root.i18n.t("当前图片无法保存草稿，编辑和导出仍可继续"));
        } else if (code === 'background-asset-missing') {
            this._autosaveEnabled = false;
            this.root.editor.message?.warning?.(this.root.i18n.t("背景资源缺失，已停止自动保存，编辑和导出仍可继续"));
        } else if (/quota/i.test(text)) {
            this._autosaveEnabled = false;
            this.root.editor.message?.warning?.(this.root.i18n.t("存储空间不足，已停止自动保存，本次编辑仍可继续"));
        } else if (/unavailable|blocked|open-failed|security|invalidstate/i.test(text)) {
            this._autosaveEnabled = false;
            this._warnStorageUnavailable();
        } else {
            this._autosaveEnabled = false;
            this.root.editor.message?.warning?.(this.root.i18n.t("本地草稿保存失败，已停止自动保存，本次编辑仍可继续"));
        }
    }
    ...
    _warnStorageUnavailable() {
        ...
        this.root.editor.message?.warning?.(this.root.i18n.t("本地草稿存储不可用，本次编辑仍可继续，但关闭页面后不会自动恢复"));
    }
    ...
    restore() {
        ...
        // Do not let edits race the asynchronous read and overwrite the draft.
    ...
    async _runRestore(key, generation) {
        ...
        try {
            ...
            for (let index = 0; index < valid.images.length; index += 1) {
                ...
                if (!imageRecord?.blob) {
                    this.root.editor.message?.info?.(this.root.i18n.t("草稿原图资源缺失，无法恢复"));
                    return false;
                }
            ...
            if (backgroundAssetId) {
                ...
                if (!backgroundRecord?.blob) {
                    this.root.editor.message?.info?.(this.root.i18n.t("草稿背景资源缺失，无法恢复"));
                    return false;
                }
                ...
                if (!backgroundRestored) {
                    this.root.editor.message?.info?.(this.root.i18n.t("草稿背景资源无法加载，无法恢复"));
                    return false;
                }
            }
            ...
            hasStoredDraft = true; // A failed read cannot prove that the key is empty.
            if (!this._isCurrent(key, generation)) return false;
            ...
            if (/image(?:-\d+)?-(invalid|type-unsupported|decode-failed|dimensions-invalid|pixels-too-large)/.test(code)) {
                this.root.editor.message?.warning?.(this.root.i18n.t("草稿图片资源已损坏或尺寸过大，已忽略"));
            } else {
                this._warnStorageUnavailable();
            }
            return false;
        } finally {
            if (this._isCurrent(key, generation)) {
                runInAction(() => {
                    this._restoring = false;
                    if (!committed && hasStoredDraft) this._protectUnrestoredDraft();
                    else if (!committed && this.root.imageStore.list.length) this._scheduleSave();
                });
            }
            if (!committed) {
    ...
    async clear() {
        if (!this.config || this.recoveryBlocked || this._restoring) return false;
        const key = this.config.key;
        this._generation += 1;
        this._blockedImageSrc = null;
        if (this._saveTimer) { clearTimeout(this._saveTimer); this._saveTimer = null; }
