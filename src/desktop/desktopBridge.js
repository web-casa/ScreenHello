...
const DESKTOP_STATE_STATUSES = new Set(['initialized', 'ready', 'migrated', 'unavailable']);

const invalidDesktopState = () => new Error('desktop-state-invalid');

export const normalizeDesktopStateStatus = (value) => {
    if (!value || typeof value !== 'object' || Array.isArray(value)) throw invalidDesktopState();
    const status = /** @type {Record<string, unknown>} */ (value);
    const keys = Object.keys(status);
    if (keys.length !== 3
        || !['schemaVersion', 'status', 'dataSchemaVersion'].every((key) => Object.hasOwn(status, key))
        || status.schemaVersion !== DESKTOP_STATE_SCHEMA_VERSION
        || !DESKTOP_STATE_STATUSES.has(status.status)
        || status.dataSchemaVersion !== 1) {
...
export const readDesktopStateStatus = async (invoke = tauriInvoke) => {
    try {
        return Object.freeze({
            status: 'ready',
            state: normalizeDesktopStateStatus(await invoke('desktop_state_status')),
        });
    } catch {
        return Object.freeze({ status: 'unavailable', reason: 'desktop-state-unavailable' });
    }
};
...
export const setDesktopLocale = async (locale, invoke = tauriInvoke) => {
    if (!localeOptions.some(({ value }) => value === locale)) throw new Error('desktop-locale-invalid');
    // Native Rust dialogs still expose a bounded bilingual contract. Additional
    // WebView locales use English there until native catalogs are localized.
    await invoke('desktop_set_locale', { locale: locale === 'zh-CN' ? 'zh-CN' : 'en-US' });
};
