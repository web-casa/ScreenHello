...
const captureErrorMessage = (error) => {
    if (error?.code === 'desktop-capture-consent-required') return '截图授权已失效，请重新打开截图选择';
    if (error?.code === 'desktop-capture-source-invalid') return '截图选择已过期或已使用，请刷新后重新授权';
    if (error?.code === 'desktop-capture-cancelled') return '已取消截图授权，未生成截图';
    if (error?.code === 'desktop-capture-system-permission-required') return 'macOS 未授予屏幕录制权限。请在“系统设置 > 隐私与安全性 > 屏幕与系统音频录制”中允许 ScreenHello，然后重新打开应用。';
    if (error?.code === 'desktop-capture-wayland-portal-required') return '当前 Wayland 会话需要 ScreenCast Portal；本版本不会回退到不可靠的窗口枚举。请改用受支持的 X11 会话，或等待 Portal 集成。';
...
const captureCapabilityMessage = (capability) => (
    capability?.status === 'system-permission-required'
        ? '需要 macOS 屏幕录制权限。确认后，系统会请求此权限。'
        : ''
);
...
export default observer(function DesktopCaptureController() {
    ...
    const loadSources = useCallback(async ({ requestSystemPermission = false } = {}) => {
        const sequence = ++requestSequence.current;
        setLoading(true);
        setError('');
        try {
            const nextCapability = await stores.platform.capture.getCapability();
            if (sequence !== requestSequence.current) return;
            setCapability(nextCapability);
            if (nextCapability.status === 'portal-required' || nextCapability.status === 'no-display') {
                setSources([]);
                setSelectedToken('');
                setError(t(captureErrorMessage({
                    code: nextCapability.status === 'portal-required'
                        ? 'desktop-capture-wayland-portal-required'
                        : 'desktop-capture-no-display',
                })));
                return;
            }
            if (nextCapability.status === 'system-permission-required' && !requestSystemPermission) {
                setSources([]);
                setSelectedToken('');
                return;
            }
            ...
            if (sequence !== requestSequence.current) return;
            setCapability({ ...nextCapability, status: 'ready', sourcePicker: true });
            ...
            const nextSource = nextSources.find((source) => source.kind === 'monitor' && source.primary)
                ...
                || nextSources[0];
            setSourceKind(nextSource?.kind || 'monitor');
            selectSource(nextSource);
        } catch (nextError) {
            if (sequence !== requestSequence.current) return;
            setSources([]);
            setSelectedToken('');
            setError(t(captureErrorMessage(nextError)));
        } finally {
            if (sequence === requestSequence.current) setLoading(false);
        }
    }, [selectSource, stores, t]);
    ...
    useEffect(() => {
        ...
        subscribeDesktopSystemEvents((event) => {
            ...
            if (interactionBlocked.current) {
                stores.editor.message?.info?.(t("请先完成或取消当前截图选择"));
                return;
            }
            void stores.commands.execute('file.captureScreen', { mode: 'primary' }).then((handled) => {
                if (active && !handled) stores.editor.message?.info?.(t("当前任务完成后再截取屏幕"));
            });
        }).then((cleanup) => {
            if (active) unsubscribe = cleanup;
            else void cleanup();
        }).catch(() => {
            if (active) stores.editor.message?.warning?.(t("系统快捷键与托盘截图入口暂时不可用；仍可使用文件菜单"));
        });
        return () => {
            active = false;
    ...
    const capabilityNotice = captureCapabilityMessage(capability);
    ...
    return (
        <Modal
            ...
            footer={[
                <Button key="cancel" disabled={capturing} onClick={closeCapture}>{t("取消")}</Button>,
                <Button key="refresh" disabled={capturing || loading} onClick={() => { void loadSources(); }}>{t("刷新来源")}</Button>,
                capability?.status === 'system-permission-required'
                    ? <Button key="request-permission" type="primary" loading={loading} disabled={capturing} onClick={() => { void loadSources({ requestSystemPermission: true }); }}>{t("请求系统权限")}</Button>
                    : null,
                <Button key="capture" type="primary" loading={capturing} disabled={!captureEnabled} onClick={() => { void submitCapture(); }}>{t("截取并添加")}</Button>,
            ].filter(Boolean)}
        >
            <p className="shoteasy-capture-dialog__intro">{t("选择显示器、应用窗口或显示器内区域。截图只在本机处理。")}</p>
            {capabilityNotice ? <Alert type="warning" showIcon message={t(capabilityNotice)} /> : null}
            {hasWindows && (
                <Segmented
                    block
                    aria-label={t("截图来源类型")}
                    value={sourceKind}
                    options={[{ label: t("显示器"), value: 'monitor' }, { label: t("窗口"), value: 'window' }]}
                    onChange={selectKind}
                />
            )}
            <div className="shoteasy-capture-dialog__sources" aria-busy={loading}>
                {loading ? <Spin tip={t("正在读取本机屏幕来源")}><div className="shoteasy-capture-dialog__loading" /></Spin> : null}
                        ...
                        {visibleSources.map((source) => (
                            <Radio key={source.token} value={source.token}>
                                <span className="shoteasy-capture-source__copy">
                                    ...
                                    <small>
                                        {source.primary ? t("主显示器 · ") : ''}{source.width} × {source.height}
                                        {source.kind === 'monitor' ? t(" · 坐标 {0}, {1} · {2}×", { 0: source.x, 1: source.y, 2: source.scaleFactor }) : ''}
                                    </small>
                                </span>
                            </Radio>
                        ))}
                ...
                {!loading && !visibleSources.length ? <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={t("没有可用来源")} /> : null}
            </div>
            {selectedSource?.kind === 'monitor' && (
                <div className="shoteasy-capture-dialog__region">
                    <Segmented
                        aria-label={t("截图范围")}
                        value={captureMode}
                        options={[{ label: t("整个显示器"), value: 'full' }, { label: t("指定区域"), value: 'region' }]}
                        onChange={(value) => { setCaptureMode(value); setError(''); }}
                    />
                    {captureMode === 'region' && (
                        <div className="shoteasy-capture-dialog__region-fields">
                            <InputNumber min={0} max={Math.max(0, selectedSource.width - 1)} value={region.x} prefix="X" aria-label={t("区域 X 坐标")} onChange={(value) => updateRegion('x', value)} />
                            <InputNumber min={0} max={Math.max(0, selectedSource.height - 1)} value={region.y} prefix="Y" aria-label={t("区域 Y 坐标")} onChange={(value) => updateRegion('y', value)} />
                            <InputNumber min={1} max={Math.max(1, selectedSource.width - region.x)} value={region.width} prefix="W" aria-label={t("区域宽度")} onChange={(value) => updateRegion('width', value)} />
                            <InputNumber min={1} max={Math.max(1, selectedSource.height - region.y)} value={region.height} prefix="H" aria-label={t("区域高度")} onChange={(value) => updateRegion('height', value)} />
                        </div>
                    )}
                </div>
            )}
