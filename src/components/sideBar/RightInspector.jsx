...
import { Theme as RadixTheme, Switch, TextField, IconButton, Button as RadixButton, SegmentedControl } from '@radix-ui/themes';
 ...
 */
/** Radix 原语版滑块：外观复用 .rt-Slider* 类名，可访问名称落在 Thumb 上。 */
function RadixSlider({ label, min, max, step = 1, value, onChange, onCommit, disabled, size = 1 }) {
    return (
        <SliderRoot
            className={`rt-SliderRoot rt-r-size-${size}`}
            min={min}
            max={max}
            step={step}
        ...
        >
            <SliderTrack className="rt-SliderTrack"><SliderRange className="rt-SliderRange" /></SliderTrack>
            <SliderThumb className="rt-SliderThumb" aria-label={label} />
        </SliderRoot>
    );
}
                ...
                <div className="flex items-center gap-2">
                    ...
                    <TextField.Root
                        ...
                        onChange={(event) => {
                            ...
                            onChange(Number.isFinite(next) ? next : (inputMin ?? min));
                        }}
                        className="shoteasy-number-field"
                        aria-label={t("{0}数值", { 0: label })}
                    />
                </div>
            </div>
            <RadixSlider label={label} min={min} max={max} step={step} value={current} onChange={onChange} disabled={disabled} />
        </div>
    );
}
...
export const InspectorContent = observer(() => {
    ...
    return (
        <RadixTheme appearance={stores.editor.isDark ? 'dark' : 'light'} accentColor="indigo" grayColor="slate" radius="medium" scaling="100%" className="shoteasy-inspector relative h-full flex flex-col">
                            ...
                            <div className="flex justify-between items-center pb-1.5">
                                ...
                                <div className="flex items-center gap-1">
                                    ...
                                    <RadixButton
                                        ...
                                        variant="ghost"
                                        color="gray"
                                        className="text-xs flex items-center m-0"
                                        onClick={() => setShowMore(true)}
                                    >{t("更多")}<Icon.ChevronRight size={16} /></RadixButton>
                                </div>
                            </div>
                            <BackgroundSelect
                                type="gradient"
                ...
                <Section title={t("图片")}>
                    <div className="pb-3">
                        ...
                        <div className="flex gap-3 items-center py-2">
                            <CropperImage />
                            <IconButton size="2" variant="soft" color="gray" aria-label={t("水平翻转")} onClick={() => stores.option.toggleFlip('x')}><Icon.FlipHorizontal2 size={18} /></IconButton>
                            <IconButton size="2" variant="soft" color="gray" aria-label={t("垂直翻转")} onClick={() => stores.option.toggleFlip('y')}><Icon.FlipVertical2 size={18} /></IconButton>
                            <Position />
                        </div>
                    </div>
                    <div className="pb-3">
                        ...
                        <RadixSlider
                            ...
                            onChange={(next) => stores.option.setScale(next, { commit: false })}
                            onCommit={() => stores.history.commit('slider:scale')}
                        />
                    </div>
                    <div className="pb-1">
                        <SliderRow
                    ...
                    <div className="pb-3">
                        ...
                        <RadixSlider
                            ...
                            onChange={(next) => stores.option.setRotation(next, { commit: false })}
                            onCommit={() => stores.history.commit('rotation')}
                        />
                    </div>
                </Section>
                        ...
                        <div className="pb-3">
                            ...
                            <div className="py-1">
                                <SegmentedControl.Root
                                    ...
                                    onValueChange={(v) => stores.option.setFrameMode(v)}
                                >
                                    <SegmentedControl.Item value="cover">{t("覆盖")}</SegmentedControl.Item>
                                    <SegmentedControl.Item value="fit">{t("包含")}</SegmentedControl.Item>
                                    <SegmentedControl.Item value="stretch">{t("拉伸")}</SegmentedControl.Item>
                                </SegmentedControl.Root>
                            </div>
                        </div>
                    )}
                        ...
                        <div className="flex justify-between items-center gap-2 pb-1">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <Switch
                                    ...
                                    onCheckedChange={(visible) => stores.option.setInnerBorder({ visible })}
                                    aria-label={t("启用内描边")}
                                />
                            </div>
                        </div>
