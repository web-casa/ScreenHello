...
import { Theme as RadixTheme, Button as RadixButton } from '@radix-ui/themes';
...
export default observer(function DownloadBar() {
    ...
    const libraryExportOptions = (
        <div className="shoteasy-export-popover">
            <div className="p-3 [&_.ant-segmented]:w-full [&_.ant-segmented-item]:flex-1">
                <div className="text-xs font-medium text-[var(--se-muted)] mb-2">{t("格式")}</div>
                <Segmented
                ...
                />
                <div className="text-xs font-medium text-[var(--se-muted)] mt-4 mb-2">{t("像素倍率")}</div>
                <Segmented
                ...
                {stores.option.frameConf.width && (
                    <div className="text-xs p-3 mt-4 flex justify-between bg-[var(--se-panel-muted)] rounded-lg">
                        <span className="text-[var(--se-muted)]">{t("导出尺寸")}</span>
    ...
    const themeProvider = {
        token: {
            // 导出抽屉被 portal 到 document.body，应用根上的 .shoteasy-app[data-mode] 选择器
            // 覆盖不到它；这里在同一个 Provider 上再给一次禁用态色，保证抽屉里的按钮一致。
        ...
        <div className="shoteasy-top-actions">
            ...
            <ConfigProvider theme={themeProvider}>
                <div className="shoteasy-workspace-mobile-only">
                    <Tooltip placement="bottom" arrow={false} title={t("批量套用同一风格并导出 ZIP")}>
                        <Button
                            type="text"
                    ...
                    <Tooltip placement="bottom" arrow={false} title={replaceCommand.disabledReason || t("替换当前图片")}>
                        <Button
                            type="default"
                            size="middle"
                    ...
                    <Tooltip placement="bottom" arrow={false} title={downloadCommand.disabledReason || t("使用当前设置快速导出 · {0}x {1}", { 0: ratio, 1: format.toUpperCase() })}>
                        <Button
                            type="default"
                            size="middle"
                            className="shoteasy-top-action"
                            ...
                            aria-label={t("使用当前设置快速导出")}
                            onClick={() => { void downloadCommand.execute(); }}
                        />
                    </Tooltip>
                    {hasImage && (
                        <Tooltip placement="bottom" arrow={false} title={newProjectCommand.disabledReason || t("清空当前项目")}>
                            <Button
                                size="middle"
                                danger
                                className="shoteasy-top-action"
                ...
                <RadixTheme appearance={stores.editor.isDark ? 'dark' : 'light'} accentColor="indigo" grayColor="slate" radius="medium" asChild>
                    <span className="shoteasy-top-actions-radix">
                        <Tooltip placement="bottom" arrow={false} title={t("复制 {0} + C", { 0: modKey })}>
                            <RadixButton
                                size="2"
                                variant="soft"
                                color="gray"
                        ...
                        <RadixButton
                            ...
                            aria-label={t("导出图片")}
                            title={exportCommand.disabledReason || t("选择格式与倍率并导出")}
                            onClick={() => { void exportCommand.execute(); }}
                        ><Icon.Download size={17} /><span className="shoteasy-export-label">{t("导出")}</span></RadixButton>
                    </span>
                </RadixTheme>
                <ExportPanel />
            </ConfigProvider>
    ...
    return (
        <div className="shoteasy-top-actions">
            ...
            />
            <ConfigProvider theme={themeProvider}>
                <Tooltip placement="bottom" arrow={false} title={t("更换图片")}>
                    <Button
                        type="default"
                        size="middle"
                        className="shoteasy-top-action shoteasy-copy-action"
                ...
                <Tooltip placement="bottom" arrow={false} title={t("下载 {0} + S · {1}x {2}", { 0: modKey, 1: ratio, 2: format.toUpperCase() })}>
                    <Button
                        type="primary"
                        size="middle"
                ...
                >
                    <Tooltip placement="bottom" arrow={false} title={t("导出格式与倍率")}>
                        <Button
                            size="middle"
                            className="shoteasy-top-action shoteasy-top-action--export"
                            disabled={!hasImage}
                            aria-label={t("导出格式与倍率（当前 {0}x {1}）", { 0: ratio, 1: format.toUpperCase() })}
                        >
