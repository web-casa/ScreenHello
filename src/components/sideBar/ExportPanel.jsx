...
export default observer(function ExportPanel() {
    ...
    const updateDraft = settings => {
        ...
        setDraft(normalizeExportSettings(validateExportSettings(settings)));
    };
    ...
    const standardOnly = draft.format === 'avif';
    ...
    return (
        <Drawer
            ...
            footer={(
                <>
                    {downloadTooLarge ? <p id={downloadLimitId} className="shoteasy-export-warning" role="status">{t('压缩下载最多支持约 419 万像素，最长边为 8192 像素。请降低倍率或画布尺寸，或改用 PNG/JPG/WebP 标准导出。')}</p> : null}
            ...
            <div className="shoteasy-export-panel">
                ...
                <section className="shoteasy-compression-settings" aria-label={t('压缩设置')}>
                    ...
                    <div className="shoteasy-compression-modes" role="group" aria-label={t('压缩方式')}>
                        {(standardOnly ? ['standard'] : ['standard', ...(['png', 'webp'].includes(draft.format) ? ['lossless'] : []), 'lossy']).map(value => (
                    ...
                    {standardOnly ? <p className="shoteasy-export-note" role="status" data-testid="avif-standard-only">{t(AVIF_STANDARD_ONLY_MESSAGE)}</p> : null}
                ...
                {!standardOnly ? <section className="shoteasy-preview-control" aria-label={t('压缩预览')}>
                    <div className="shoteasy-export-panel__heading"><strong>{t('压缩预览')}</strong><span>{t('可选；调整参数后不会自动重新生成。')}</span></div>
                    <Button block disabled={busy || previewTooLarge} loading={preview.state === 'preparing'}
                        onClick={() => { void preview.generate(draft); }} data-testid="export-preview">
                ...
                {!standardOnly && ready ? <ExportPreviewResult prepared={preview.prepared} disabled={busy} onFailure={preview.fail} /> : null}
