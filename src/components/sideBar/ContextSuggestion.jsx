    ...
    if (state.status === 'analyzing' || state.status === 'idle') {
        return (
            <div className="shoteasy-context-suggestion is-pending" aria-busy={state.status === 'analyzing'}>
                <span className="shoteasy-context-suggestion__eyebrow">{t("本地建议")}</span>
                <span>{state.status === 'analyzing' ? t("正在此设备分析…") : t("等待分析图片")}</span>
            </div>
        );
    }
    ...
    if (state.status === 'unavailable' || !state.result) {
        return (
            <div className="shoteasy-context-suggestion is-error">
                <span>{t('暂时无法生成{0}建议', { 0: label })}</span>
                <Button
                    type="link"
                    size="small"
                    disabled={busy}
                    aria-label={busy ? t("{0}本地建议暂不可用：正在处理其他本地任务", { 0: label }) : t("重新分析{0}本地建议", { 0: label })}
                    onClick={retry}
                >
                    {busy ? t("处理中") : t("重试")}
                </Button>
    ...
    return (
        <div className="shoteasy-context-suggestion" data-suggestion-kind={kind}>
            ...
            <span className="shoteasy-context-suggestion__copy">
                <span className="shoteasy-context-suggestion__eyebrow">{t("本地建议")}</span>
                <strong>{kind === 'frame' ? t(presentation.value) : presentation.value}</strong>
            </span>
            <Button
                type="link"
                size="small"
                ...
                aria-label={applied
                    ? t("{0}本地建议已应用", { 0: label })
                    : busy
                        ? t("{0}本地建议暂不可用：正在处理其他本地任务", { 0: label })
                        : t("应用{0}本地建议", { 0: label })}
                onClick={() => stores.workspace.applySuggestion(kind)}
            >
                {applied ? t("已应用") : busy ? t("处理中") : t("应用")}
            </Button>
        </div>
    );
});
