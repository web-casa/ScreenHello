...
export default observer(function ImageLayersPanel() {
    ...
    const disabledOrderReason = (direction) => {
        if (!hasSelection) return t("请先选择图层");
        if (hasLockedSelection) return t("请先解锁选中图层");
        if (orderBusy) return t("正在处理其他本地任务");
    ...
    const reorder = (direction) => {
        ...
        if (moved) setOrderAnnouncement(t("已{0}：{1} 个图层", { 0: t(ORDER_LABELS[direction]), 1: selectedLayers.length }));
        return moved;
    };
    ...
    const handleLayerKeyDown = (event, layer) => {
        ...
        if (orderBusy || layer.locked || selectedBlockContainsLocked(layer.id)) {
            setOrderAnnouncement(t("{0}：正在处理任务或选中项含锁定图层，无法调整层级", { 0: layer.name }));
            return;
        }
        ensureSelected(layer.id);
        const moved = stores.imageStore.reorderSelected(direction);
        if (moved) setOrderAnnouncement(`${layer.name}: ${t(ORDER_LABELS[direction])}`);
    };
    ...
    const handleDrop = (event, target) => {
        ...
        if (moved) setOrderAnnouncement(t("已将 {0} 个图层移到“{1}”{2}", { 0: count, 1: target.name, 2: position === 'above' ? t('上方') : t('下方') }));
        clearDrag();
    };
    ...
    return (
        <div className="shoteasy-layer-panel">
            ...
            <div className="shoteasy-layer-actions is-primary">
                ...
                >{t("添加图片")}</Button>
                <Action label={t("复制图层")} disabled={!duplicateCommand.enabled} onClick={() => { void duplicateCommand.execute(); }}><Icon.Copy size={14} /></Action>
                <Action label={t("删除图层")} disabled={!deleteCommand.enabled} onClick={() => { void deleteCommand.execute(); }}><Icon.Trash2 size={14} /></Action>
            </div>
            <div className="shoteasy-layer-summary" role="status" aria-live="polite">
                <strong>{t('已选 {0} / 共 {1} 层', { 0: selectedLayers.length, 1: layers.length })}</strong>
                <span>{hasLockedSelection ? t("选中项含锁定图层") : t("拖动可排序")}</span>
            </div>
            <div className="shoteasy-layer-list" role="list" aria-label={t("图片图层")} aria-describedby={`${stores.id}-layer-order-help`}>
                {layers.map((layer) => {
                    ...
                    >
                        ...
                        >
                            ...
                            <span className="shoteasy-layer-state">
                                <small>{layer.groupId ? t("组") : ''}</small>
                                <small aria-label={layer.locked ? t("已锁定") : t("未锁定")}>{layer.locked ? t("锁") : ''}</small>
                            </span>
            ...
            {hasSelection && <p id={`${stores.id}-layer-order-help`} className="shoteasy-layer-order-help">{t("拖动图层调整层级；也可使用下方按钮，或在图层上按 Alt + 方向键。")}</p>}
            <span className="sr-only" role="status" aria-live="polite">{orderAnnouncement}</span>
            <div className="shoteasy-layer-actions">
                <Action label={hasGroup ? t("取消编组") : t("编组")} disabled={hasGroup ? !ungroupCommand.enabled : !groupCommand.enabled} onClick={() => { void (hasGroup ? ungroupCommand.execute() : groupCommand.execute()); }}>{hasGroup ? t("解组") : t("编组")}</Action>
                <Action label={allLocked ? t("解锁图层") : t("锁定图层")} disabled={!lockCommand.enabled} onClick={() => { void lockCommand.execute(); }}>{allLocked ? t("解锁") : t("锁定")}</Action>
            </div>
            {/* 未选中图层时收起排序与对齐：整排禁用按钮只剩"一片空框"，
                既占地方又看不出能做什么。选中后才展开这些能力。 */}
            {hasSelection && <div className="shoteasy-layer-actions is-order">
                {['top', 'up', 'down', 'bottom'].map((direction) => {
                    const disabledReason = disabledOrderReason(direction);
                    return (
                        <Action
                            key={direction}
                            label={t(ORDER_LABELS[direction])}
                            disabled={Boolean(disabledReason)}
                            disabledReason={disabledReason}
                            onClick={() => reorder(direction)}
                        >
                            {{ top: t("置顶"), up: t("上移"), down: t("下移"), bottom: t("置底") }[direction]}
                        </Action>
                    );
                })}
            </div>}
            {hasSelection && <div className="shoteasy-layer-actions is-grid">
                {['left', 'center', 'right', 'top', 'middle', 'bottom'].map((axis) => (
                    <Action key={axis} label={t("对齐 {0}", { 0: axis })} disabled={!hasSelection || allLocked} onClick={() => stores.imageStore.alignSelected(axis)}>
                        {{ left: t("左"), center: t("水平中"), right: t("右"), top: t("上"), middle: t("垂直中"), bottom: t("下") }[axis]}
                    </Action>
                ))}
                <Action label={t("水平等间距")} disabled={selectedLayers.length < 3 || allLocked} onClick={() => stores.imageStore.distributeSelected('horizontal')}>{t("水平分布")}</Action>
                <Action label={t("垂直等间距")} disabled={selectedLayers.length < 3 || allLocked} onClick={() => stores.imageStore.distributeSelected('vertical')}>{t("垂直分布")}</Action>
                <Action label={t("堆叠布局")} disabled={selectedLayers.length < 2 || allLocked} onClick={() => stores.imageStore.stackSelected()}>{t("堆叠")}</Action>
                <Action label={t("扇形布局")} disabled={selectedLayers.length < 2 || allLocked} onClick={() => stores.imageStore.fanSelected()}>{t("扇形")}</Action>
            </div>}
        </div>
    );
});
