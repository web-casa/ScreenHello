...
const DeviceSource = ({ device }) => {
    ...
    return <div className="shoteasy-device-license">
        ...
        {device.sourceProject === 'Devices.css'
            ? <p>{t('机身来自 MIT 许可的 Devices.css，构建期渲染为透明 PNG；导出图片不附加文字。')}</p>
...
export default observer(function FrameBar() {
    ...
    // 抽屉关闭后把焦点还给触发按钮，但只在没有别的浮层开着的时候：
    // rc-drawer 的 focusTriggerAfterClose 会在自己的过渡结束时无条件归还，
    // 那个回调可能晚于另一个浮层（例如导出抽屉）的焦点归还，把焦点抢回本按钮。
    const restoreMoreFocus = () => {
        const trigger = moreTriggerRef.current;
        if (!trigger?.isConnected) return;
        if (globalThis.document?.querySelector('.ant-drawer-open, .ant-modal-wrap .ant-modal:not([style*="display: none"])')) return;
    ...
    return (
        <>
            <section className="shoteasy-frame-panel" aria-labelledby={`${frameIdPrefix}-panel-title`}>
                ...
                <Button block className="shoteasy-frame-more" ref={moreTriggerRef} onClick={() => setShowMore(true)}
            ...
            <Drawer
                ...
                afterOpenChange={(open) => { if (!open) restoreMoreFocus(); }}
