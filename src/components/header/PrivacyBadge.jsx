...
/**
 * 顶栏隐私计数器：显示本次会话已知的公网发送字节数。
 *
 * 数字来自运行时出口监控（fetch / XHR / sendBeacon / WebSocket），不是静态文案：
 * 一旦检测到公网发送负载，这里会告警；浮层始终给出请求次数与最近地址。
...
const PrivacyDetails = observer(function PrivacyDetails() {
    ...
    return (
        <div className="shoteasy-privacy-details">
            <dl>
                <div><dt>{t('公网发送 {0}', { 0: privacy.formattedUploadedBytes })}</dt><dd>{t('公网发送数据量')}</dd></div>
                <div><dt>{t('公网请求 {0} 次', { 0: privacy.remoteRequests })}</dt><dd>fetch / XHR / beacon / WebSocket</dd></div>
            ...
            {privacy.unknownBody > 0 && (
                <p role="alert">{t('有 {0} 次公网请求的请求体长度无法确定。', { 0: privacy.unknownBody })}</p>
            )}
            {privacy.remoteRequests > 0 && privacy.lastRemote && (
                <p role="alert">{t('最近一次公网请求：{0}', { 0: privacy.lastRemote })}</p>
            )}
            <p>{t('图片、项目、草稿与预设都在本机处理，这里仅显示运行时统计到的公网出口流量。')}</p>
        </div>
    );
});
...
export default observer(function PrivacyBadge() {
    ...
    const label = t('公网发送 {0}', { 0: privacy.formattedUploadedBytes });
    return (
        <Popover
