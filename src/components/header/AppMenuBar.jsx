...
export default observer(function AppMenuBar() {
    ...
    useLayoutEffect(() => {
        ...
        // WebKit can restore focus to the clicked trigger after this layout
        // effect. Re-apply it on the next frame while the same menu is still
        // open, so pointer and keyboard opening share one reliable focus path.
        const frame = globalThis.requestAnimationFrame?.(() => {
            if (openMenuRef.current === openMenu) focusPopup(popupRefs.current[openMenu]);
        });
        return () => {
    ...
    const recentChildren = stores.workspace.libraryStatus === 'loading'
        ? [{ key: 'recent-loading', disabled: true, label: t("正在读取本地项目…") }]
        : stores.workspace.libraryStatus === 'unavailable'
            ? [{ key: 'recent-unavailable', disabled: true, label: t("本地资料库不可用") }]
            : stores.workspace.recentProjects.slice(0, 12).map((item) => (
                commandItem('file.openRecentProject', { id: item.id }, `recent:${item.id}`)
            ));
    if (!recentChildren.length) recentChildren.push({ key: 'recent-empty', disabled: true, label: t("暂无最近项目") });

    const menus = {
        file: [
            commandItem('file.newProject'),
            commandItem('file.openProject'),
            { key: 'recent-projects', label: t("最近项目"), children: recentChildren },
    ...
    const mobileTabs = MENU_IDS.map((menuId) => ({
        key: menuId,
        label: t(MENU_LABELS[menuId]),
        children: (
            <Menu
                aria-label={t("{0}命令", { 0: t(MENU_LABELS[menuId]) })}
                mode="inline"
                selectable={false}
                items={menus[menuId]}
    ...
    return (
        <>
            <nav className="shoteasy-app-menu" role="menubar" aria-label={t("应用菜单")}>
