...
const CAPTURE_ERROR_CODES = new Set([
    'desktop-capture-consent-required',
    'desktop-capture-consent-unavailable',
    ...
    'desktop-capture-capability-invalid-response',
]);
...
/** @param {unknown} value @returns {DesktopCaptureCapability} */
const normalizeCaptureCapability = (value) => {
    const response = /** @type {any} */ (value);
    if (!value || typeof value !== 'object' || Array.isArray(value)
        || Object.keys(response).length !== 4
        || !Object.hasOwn(response, 'schemaVersion') || !Object.hasOwn(response, 'backend')
        || !Object.hasOwn(response, 'status') || !Object.hasOwn(response, 'sourcePicker')
        || response.schemaVersion !== 1
        || !CAPTURE_CAPABILITY_BACKENDS.has(response.backend)
        || !CAPTURE_CAPABILITY_STATUSES.has(response.status)
        || typeof response.sourcePicker !== 'boolean'
        || response.sourcePicker !== (response.status === 'ready')) {
        throw desktopError('desktop-capture-capability-invalid-response');
    }
    return Object.freeze({
        schemaVersion: 1,
        backend: /** @type {DesktopCaptureBackend} */ (response.backend),
        status: /** @type {DesktopCaptureCapabilityStatus} */ (response.status),
        sourcePicker: response.sourcePicker,
    });
};
...
export const createDesktopPlatform = ({
    ...
    const platform = {
        ...
        capture: {
            ...
            async getCapability() {
                try {
                    return normalizeCaptureCapability(await invokeCommand('desktop_capture_capability'));
                } catch (error) {
                    if (error && typeof error === 'object' && 'code' in error
            ...
            async listSources() {
                try {
                    return normalizeCaptureSources(await invokeCommand('desktop_list_capture_sources', { requestConsent: true }));
                } catch (error) {
