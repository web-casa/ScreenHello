 ...
 * 「本机」或「外部」，并统计**可精确测量的外部请求体字节数**。UI 只读这些计数，所以
 * 「0 B 数据已上传」是运行时结论，不是静态文案：一旦有数据发往外部源，数字立刻变化。
 *
 * 边界（刻意写清楚，避免过度承诺）：
 * - 覆盖发送数据的 API：fetch、XMLHttpRequest、navigator.sendBeacon、WebSocket 连接与 send。
 * - 「本机」= 同源、`localhost`/`127.0.0.1`/`::1` 回环、`blob:`、`data:`、`file:`、`about:`、相对地址。
 ...
 */
const LOCAL_PROTOCOLS = new Set(['blob:', 'data:', 'file:', 'about:', 'filesystem:']);
...
/** 判断目标地址是否属于「不出本机」。无法解析的地址按外部处理（保守）。 */
export const isLocalTarget = (rawUrl, origin = currentOrigin()) => {
    ...
    if (['http:', 'https:', 'ws:', 'wss:'].includes(url.protocol)) {
        const comparableOrigin = url.origin.replace(/^ws:/, 'http:').replace(/^wss:/, 'https:');
        if (comparableOrigin === base) return true;
