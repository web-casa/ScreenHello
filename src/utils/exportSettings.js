...
// Product-facing settings only. Keep the archive/codec contract above readable
// for older files and direct library callers; never change format or scale.
export const AVIF_STANDARD_ONLY_MESSAGE = 'AVIF 仅提供标准导出，不提供压缩预览或质量调节。旧压缩设置将使用标准导出，格式和倍率不变。';
export function normalizeEditorExportSettings(value = {}) {
    const settings = normalizeExportSettings(value);
    return settings.format === 'avif' ? { format: settings.format, ratio: settings.ratio } : settings;
}
