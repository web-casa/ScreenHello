...
// 所有可着色源图标都把中性占位色放在 fill/stroke 属性中；测试会锁住这份输入契约。
// 只替换绘制属性，避免将未来的 title、注释或其他元数据中的同一字符串误改成颜色。
const tintSvg = (source, color) => svgToDataURL(source.replace(/\b(fill|stroke)=(["'])#737373\2/g, `$1="${color}"`));
...
/**
 * 位图设备机身用自身比例决定布局基准，图片再按 frameMode 贴进屏幕；其余外框沿用图片尺寸。
 * 旧版无品牌矢量（generic*）保持按图片尺寸布局，避免改变已保存项目的外观。
 */
export const getFrameContentBasis = (frame) => {
    const definition = getFrameDefinition(frame);
    return definition.kind === 'raster-device' ? { width: definition.width, height: definition.height } : null;
};
