...
#[derive(Clone, Copy, Debug, PartialEq, Eq, Serialize)]
#[serde(rename_all = "kebab-case")]
// The IPC schema is shared by three target-specific implementations. A native
// target intentionally constructs only its own backend variant.
#[allow(dead_code)]
enum CaptureBackend {
    X11,
    WaylandPortal,
...
#[derive(Clone, Copy, Debug, PartialEq, Eq, Serialize)]
#[serde(rename_all = "kebab-case")]
// See CaptureBackend: the finite status schema is deliberately cross-platform.
#[allow(dead_code)]
...
pub(crate) struct CaptureCapability {
    schema_version: u8,
...
impl CaptureCapability {
    fn with_status(backend: CaptureBackend, status: CaptureCapabilityStatus) -> Self {
        Self {
            schema_version: 1,
            backend,
            status,
            source_picker: matches!(status, CaptureCapabilityStatus::Ready),
        }
    }
}
...
fn request_macos_screen_capture_access() -> bool {
    macos_screen_capture_access_granted()
        || (CGRequestScreenCaptureAccess() && macos_screen_capture_access_granted())
}

#[cfg(target_os = "linux")]
fn current_capture_capability() -> CaptureCapability {
    linux_capture_capability(is_wayland_session(), x11_display_is_available())
}

#[cfg(target_os = "linux")]
fn linux_capture_capability(
    wayland_session: bool,
    x11_display_available: bool,
) -> CaptureCapability {
    if wayland_session {
        return CaptureCapability::with_status(
            CaptureBackend::WaylandPortal,
            CaptureCapabilityStatus::PortalRequired,
        );
    }
    if !x11_display_available {
        return CaptureCapability::with_status(
            CaptureBackend::X11,
            CaptureCapabilityStatus::NoDisplay,
        );
    }
    CaptureCapability::with_status(CaptureBackend::X11, CaptureCapabilityStatus::Ready)
}
...
fn current_capture_capability() -> CaptureCapability {
    CaptureCapability::with_status(
        ...
        if macos_screen_capture_access_granted() {
            ...
            CaptureCapabilityStatus::SystemPermissionRequired
        },
    )
}
...
fn current_capture_capability() -> CaptureCapability {
    CaptureCapability::with_status(CaptureBackend::WindowsGdi, CaptureCapabilityStatus::Ready)
}

#[cfg(target_os = "macos")]
fn resolve_system_capture_permission(request_system_permission: bool) -> Result<(), String> {
    if request_system_permission && request_macos_screen_capture_access() {
        Ok(())
    } else {
        Err(error_code("desktop-capture-system-permission-required"))
    }
}

#[cfg(not(target_os = "macos"))]
fn resolve_system_capture_permission(_request_system_permission: bool) -> Result<(), String> {
    Err(error_code("desktop-capture-system-permission-required"))
}

fn ensure_capture_ready(request_system_permission: bool) -> Result<(), String> {
    match current_capture_capability().status {
        CaptureCapabilityStatus::Ready => Ok(()),
        CaptureCapabilityStatus::SystemPermissionRequired => {
            resolve_system_capture_permission(request_system_permission)
        }
        CaptureCapabilityStatus::PortalRequired => {
            Err(error_code("desktop-capture-wayland-portal-required"))
        }
        CaptureCapabilityStatus::NoDisplay => Err(error_code("desktop-capture-no-display")),
    }
}
...
#[tauri::command]
pub(crate) fn desktop_capture_capability() -> CaptureCapability {
    current_capture_capability()
}
...
pub(crate) async fn desktop_list_capture_sources<R: Runtime>(
    ...
    match current_capture_capability().status {
        CaptureCapabilityStatus::PortalRequired => {
            return Err(error_code("desktop-capture-wayland-portal-required"));
        }
        CaptureCapabilityStatus::NoDisplay => {
            return Err(error_code("desktop-capture-no-display"));
        }
        CaptureCapabilityStatus::Ready | CaptureCapabilityStatus::SystemPermissionRequired => {}
    }
...
mod tests {
    ...
    #[test]
    fn capture_capability_is_bounded_and_derives_picker_access_from_readiness() {
        let ready =
            CaptureCapability::with_status(CaptureBackend::X11, CaptureCapabilityStatus::Ready);
        let portal = CaptureCapability::with_status(
            CaptureBackend::WaylandPortal,
            CaptureCapabilityStatus::PortalRequired,
        );
        let permission = CaptureCapability::with_status(
            CaptureBackend::MacosCoreGraphics,
            CaptureCapabilityStatus::SystemPermissionRequired,
        );
        let no_display = CaptureCapability::with_status(
            CaptureBackend::WindowsGdi,
            CaptureCapabilityStatus::NoDisplay,
        );

        for (capability, backend, status, source_picker) in [
            (ready, "x11", "ready", true),
            (portal, "wayland-portal", "portal-required", false),
            (
                permission,
                "macos-core-graphics",
            ...
            (no_display, "windows-gdi", "no-display", false),
        ] {
            let value = serde_json::to_value(capability).unwrap();
            let object = value.as_object().unwrap();
            assert_eq!(object.len(), 4);
            assert_eq!(object.get("schemaVersion").unwrap(), 1);
            assert_eq!(object.get("backend").unwrap(), backend);
            assert_eq!(object.get("status").unwrap(), status);
            assert_eq!(object.get("sourcePicker").unwrap(), source_picker);
            assert!(object.get("display").is_none());
    ...
    #[cfg(target_os = "linux")]
    #[test]
    fn linux_capability_blocks_wayland_before_x11_and_reports_headless_x11() {
        let wayland = linux_capture_capability(true, true);
        assert_eq!(wayland.backend, CaptureBackend::WaylandPortal);
        assert_eq!(wayland.status, CaptureCapabilityStatus::PortalRequired);
        assert!(!wayland.source_picker);

        let headless = linux_capture_capability(false, false);
        assert_eq!(headless.backend, CaptureBackend::X11);
        assert_eq!(headless.status, CaptureCapabilityStatus::NoDisplay);
        assert!(!headless.source_picker);

        let x11 = linux_capture_capability(false, true);
        assert_eq!(x11.backend, CaptureBackend::X11);
        assert_eq!(x11.status, CaptureCapabilityStatus::Ready);
        assert!(x11.source_picker);
    }
}
