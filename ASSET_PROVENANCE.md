...
## Devices.css device shells (MIT, committed) — 2026-09-11

`src/assets/devicescss/` holds 18 device models (35 colour variants, 88 PNGs) rendered
from the MIT-licensed Devices.css v0.2.0 (`vendor/devices.css/`, Yan Zhu) by
`scripts/generate-devicescss-assets.mjs`. For each variant the script renders the CSS
shell and a thumbnail, measures the `.device-screen` box in the DOM, and renders one
screen alpha mask per model at the device's own size, so the existing bitmap device
pipeline (projection + mask clipping) needs no hand-authored geometry. Shells are
capped at 1440 px on the longest edge, thumbnails at 320 px.

Because the upstream stylesheet is MIT, these derivatives may be redistributed with
the source and the built applications (unlike the optional Surface/Monkr/Shoteasy
packs above). Naming follows the upstream stylesheet: real model names are used
nominatively, and the MacBook shells include the label the stylesheet draws. No
pre-download licence confirmation is shown for this family; the editor still shows
the author, source and licence links.
...
The generator's separate `site.webmanifest` is not installed: this project
generates its own manifest through `vite-plugin-pwa`, and `scripts/audit-pwa-build.mjs`
requires exactly one `rel="manifest"` link and no `site.webmanifest`.
The 2026-09-12 replacement repointed the six icons, the master SVG and every
derived icon to a new RealFaviconGenerator package; the previous package is
retired.
...
- `src/assets/logo.svg` is the project-supplied ScreenHello identity source
  provided for the 2026-09-12 brand replacement. It is the mark embedded in the
  1fa47eb9 RealFaviconGenerator package's `favicon.svg`, retained byte-for-byte
  apart from dropping the generator's fixed 356px `width`/`height` so viewBox
  scaling governs every renderer: SHA-256
  `a8858b539df4d4ac1663532e0ecb3e6b748d3dd743be73bc449cee37c43f6b91`.
- `src/assets/logo.png`, `src/assets/favicon.png`, `public/pwa-*.png` and
  `src-tauri/icons/*` derive from that one SVG through
  `scripts/generate-brand-assets.mjs` and the pinned Tauri CLI 2.11.4. This mark
  is round, centred and already carries its own visible padding, so UI,
  compatibility and desktop icons render the source viewport as-is; maskable
  icons scale the mark about the viewport centre (`0.4 / 0.579`) on an opaque
  `#111318` base so no artwork pixel leaves the safe circle. Paths and gradient
  colors are not redrawn. The SVG replaces the previous identity artwork and
  does not add a third-party icon pack.
  See [regeneration instructions](DOCS/development.md#品牌图标).
